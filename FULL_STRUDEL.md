...
- [ ] Add a headless-browser (or `OfflineAudioContext` polyfill) audio oracle so the Web Audio-rendered superdough paths can be golden-tested sample-for-sample, not just smoke-tested. The current node→Rust oracles (ZZFX/LFO/ADSR/distortion) only cover pure-JS math; superdough's `OscillatorNode`/`BiquadFilterNode`/`WaveShaperNode`/reverb/delay/phaser/vowel render inside a real Web Audio graph that does not exist in plain Node, so this needs an `OfflineAudioContext` rendered under a headless browser (e.g. Playwright/Puppeteer) or a faithful WebAudio implementation, captured to PCM and committed as goldens. Big, separate infrastructure item; until then those paths stay on behavioral smoke tests (see the audio-golden item above).
