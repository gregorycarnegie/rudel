...
- [~] Add audio golden tests where deterministic output is possible, and smoke tests for real-time outputs where exact samples are not stable. The audio-golden methodology is established: `crates/rudel-dsp/tests/zzfx_golden.rs` compares rudel's ZZFX `build_samples` against the real superdough `buildSamples` **sample-for-sample** (14 cases, generated by `tools/oracle/gen_zzfx_oracle.mjs`, the first node→Rust audio oracle alongside the existing reference-formula DSP tests for distortion/filters/drums). The LFO modulator (`lfo_golden.rs`) and the **linear ADSR gain envelope** (`adsr_golden.rs`) are also golden-tested: `tools/oracle/gen_adsr_oracle.mjs` drives superdough's verbatim `getParamADSR(..., 0, 1, 0, duration, 'linear')` through a mock Web Audio param, samples the resulting automation curve, and `adsr_value` is compared against it across 9 cases — including the two edge cases superdough cuts at the note duration (`attack > duration`, `attack + decay > duration`), which fixed a divergence where rudel's `adsr_value` kept rising/decaying past `hold_end` instead of releasing from the reached value. Still pending: extending deterministic goldens to the other synth/effect paths and adding real-time smoke tests.
