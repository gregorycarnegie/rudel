...
- [ ] Match every export and test in `edo`, including intervals, ratios, pitch naming, and EDO scale behavior. The `edo` package's sole registered export is `edoScale` (MOS large/small-step scale notation, e.g. `C:LLsLLLs:2:1`), built on its own `EdoScale`/`Intervals`/`Pitches` classes (~628 lines across `edo`/`edoscale`/`intervals`/`pitches`/`ratios`). This is a self-contained system not yet ported (rudel currently only has xen's `edo` *function*, which yields octave ratios). Deferred as a standalone follow-up.
