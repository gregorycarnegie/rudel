...
A tonal/xen parity oracle was added in this pass: `tools/oracle/gen_tonal_oracle.mjs` dumps pitch output from the real `@strudel/tonal` + `@strudel/xen` engines (37 labelled cases) and `crates/rudel-mini/tests/tonal_parity.rs` rebuilds each with rudel-core and compares hap timing exactly and pitch within tolerance. Note-name values are normalised to MIDI on both sides — rudel intentionally emits MIDI note **numbers** where Strudel emits enharmonic note-name **strings** (and rudel-core inlines the scale/chord/interval tables instead of depending on `@tonaljs/tonal`), so the comparison is musical-pitch equivalence, not string identity.
...
- [~] Verify Rudel's generated tune table against `xen/tunejs.js` and upstream scale data. The oracle spot-checks the generated `tune_table.rs` against the real Tune.js output (`hexany15` ratios via both `tune` and `xen`, exact to 10 significant figures), and a unit test guards the table stays sorted for binary search. Still pending: an exhaustive scale-by-scale diff of every Tune.js archive scale (only the names exercised by the oracle are currently proven).
- [~] Match all examples from Strudel's xenharmonic docs. The oracle reconstructs the core `xen`/`ftrans`/`withBase`/`tune` doc examples (the 31edo minor triad `i("0 8 18").xen("31edo")`, the equivalent explicit-ratio form, `hexany15`, the `<5edo 12edo>` scale alternation, `ftrans` with `step:edo`, and `withBase`) hap-for-hap. Still pending: the remaining prose doc snippets (e.g. the full `withBase` chord progressions and `tunejs`-scale showcase) as a runnable corpus.
...
- [~] Match every export in `tonal/tonleiter.mjs`, including chord tokenization, pitch-class conversion, scale-step behavior, named scales, voicing rendering, and note transposition. Scale-step placement (`scaleStep`/`stepInNamedScale` via `scale_step`/`step_in_named_scale`, incl. the `anchor` realignment), named-scale intervals, note→MIDI/pitch-class conversion, and chord tokenization (`chord_notes`/`chord_symbol`) are implemented and oracle/unit-tested. Still pending: `renderVoicing` (the chord-voicing renderer) and its `below`/`duck`/`above`/`root` anchor modes — deferred with the `voicings.mjs` item below.
...
- [~] Add visual parity tests: pure geometry/unit tests for pianoroll value/time mapping, pitchwheel angle mapping, spiral coordinates/fade, color conversion, and scheduler draw-window memory; plus scripted UI or screenshot smoke tests for inline widgets and animation repainting. Unit coverage now checks widget option parsing/sizing, widget tag filtering, pianoroll value priority and time mapping, pitchwheel frequency-angle mapping, spiral polar coordinates, and hex color/alpha handling. Pending: fade/scheduler-memory tests, broader option-combination tests, analyzer visuals, and scripted UI/screenshot smoke tests.
