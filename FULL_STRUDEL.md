...
- [~] Match Strudel's higher-order function behavior for callbacks, pattern-of-functions, and pattern-valued functions. **Callbacks** (`Pattern -> Pattern`): fully supported via the `Callback` plumbing — `every`/`firstOf`/`lastOf`/`chunk`/`jux`/`juxBy`/`superimpose`/`sometimes(By)`/`often`/`rarely`/`some(Cycles)`/`off`/`within`/`inside`/`outside`/`apply`/`always`/`never`/`layer` all run a Koto function value (`rev`, `|x| x.fast(2)`). The VM isn't `Send`/`Sync`, so callbacks are applied **eagerly** at construction, not in the query path (already noted for `fmap`/`arpWith`, which probe). **Patterned arguments**: `every`/`firstOf`/`lastOf` now take a patterned cycle count (`every("<2 4>", rev)`, sampled once per cycle by `every_pat` → `every_cycles` + `inner_join`, matching Strudel's `register` patternification — the callback applies to the whole pattern once, then a patterned count places it); `off`'s time and the arithmetic ops' operand are patternified via the existing `late`/`appLeft` paths (`off("<.25 .5>", …)` verified against Strudel). **Boolean patterns**: a bare Koto `true`/`false` now reifies to `pure(Bool)` (`reify(true)`), fixing `when(true, …)`/`struct(true)`/`mask(true)`/`pat(true)` (previously silence). Verified hap-for-hap against Strudel. Deferred/intentionally different: a **pattern-of-functions** as the transform argument (Strudel's `apply(slowcat(rev, fast2))`, where the function varies per cycle in the query path) is unsupported because the Koto VM can't run during query — callbacks bind to one eagerly-applied function; the raw bind/join family (`bind`/`innerBind`/`outerBind`/`squeezeBind`/`polyBind`/`stepBind`) isn't exposed standalone for the same reason (the high-level combinators and the `pick` family cover the reachable cases); and patterned arguments for `chunk`/`chunkBack`/`inside`/`outside`/`sometimesBy`/`juxBy`/`someCyclesBy` and `within`'s bounds still take scalars (their transform or window depends on the arg, so a patterned form needs per-cycle probing — not yet done).
