...
- [x] Match `draw/animate.mjs`: `animate`, visual shape params (`x`, `y`, `w`, `h`, `angle`, `r`, `fill`, `smear`), `rescale`, `moveXY`, `zoomIn`, smear/clear behavior, sync mode status, and any intentional difference for arbitrary callback support. — Intentionally unsupported (documented in [`docs/UNSUPPORTED.md`](docs/UNSUPPORTED.md)). `animate` is built on the `draw.mjs` arbitrary-callback runtime: it registers a per-frame JS painter that draws shapes from patterned params onto the global canvas. Rudel never runs the Koto VM from the draw/realtime path, so there is no native equivalent surface and `animate` produces no visuals; the supported scheduler-time visuals are the built-in inline widgets (`_pianoroll`/`_punchcard`/`_wordfall`/`_pitchwheel`/`_spiral`).
...
- [x] Match `tidal`, `mondo`, and `mondough` packages or document them as intentionally unsupported. — Intentionally unsupported (documented in [`docs/UNSUPPORTED.md`](docs/UNSUPPORTED.md)). These are alternative *source languages* that compile to the same pattern engine: `@strudel/tidal` (`initTidal`/`tidal`, a Haskell-flavoured TidalCycles interpreter), `@strudel/mondo` (`mondo`/`mondolang`, a Lisp-like language → JS), and `@strudel/mondough` (Mondo notation for Strudel). Rudel's authoring surface is Koto + Strudel-style mini-notation (`crates/rudel-mini`); porting parallel parser front-ends is out of scope (different language choice, not new musical capability). No native equivalent surface.
