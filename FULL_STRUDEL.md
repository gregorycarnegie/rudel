...
- [ ] Add an evaluation metadata object in `rudel-lang`, analogous to Strudel's `afterEval(options.meta)`: return `Pattern`, `SampleEffects`, source/mini locations, widget configs, labels/block metadata, and cleanup hints from a single `EvalResult` instead of only `(Pattern, SampleEffects)`.
- [ ] Port `plugin-widgets.mjs` scanning/rewrite semantics for `slider(...)`: detect only real top-level `slider` calls (not strings/comments), compute `from`/`to` from the first argument, build the stable `from:to` id, collect `{ type: "slider", from, to, id, value, min, max, step }`, and rewrite to `slider_with_id(id, value, min?, max?, step?)`.
- [ ] Port `plugin-widgets.mjs` registered widget method semantics: maintain the registered widget method list, detect `_pianoroll`/`_punchcard`/`_spiral`/`_scope`/`_pitchwheel`/`_spectrum`/`_wordfall`, collect `{ from, to, index, type, id }`, inject the stable widget id as first argument, and preserve block-eval `nodeOffset` behavior.
- [ ] Add a query-time live-value registry for editor UI controls, mirroring Strudel's `sliderValues` + `ref(() => sliderValues[id])` model. `slider_with_id` must sync code value to registry at eval time and return a continuous pattern that reads the current registry value during scheduler queries.
- [ ] Add native editor decoration/range state equivalent to CodeMirror `StateField` + `DecorationSet`: remap widget, slider, flash, and source-location ranges across edits; support full-document updates and range-aware block-eval updates that preserve decorations outside the evaluated range.
- [ ] Add a native inline slider host equivalent to `slider.mjs`: dedupe sliders by source range, draw a small inline/anchored control, update the source numeric literal while dragging, update the live registry immediately, and keep the widget's `from`/`to` positions current after document changes.
- [ ] Add a native inline/block widget host equivalent to `widget.mjs`: dedupe by `(type, id)`, create/reuse per-id canvas-like surfaces, place widgets at `to || from`, preserve outside-range widgets for block eval, and clean up removed widget resources.
- [ ] Port Strudel's block utilities (`getBlockRegions`, `getBlockAt`, `evalBlock`): blank-line-separated blocks, cursor-based block selection, block flashing, range-aware eval, label metadata, and block-local widget/mini-location updates.
- [ ] Port CodeMirror settings compartments as Rudel editor settings: line wrapping, bracket matching, bracket closing, line numbers, active line, autocomplete, pattern highlighting, flash, tooltips, tab indentation, multi-cursor, theme, font family, and font size.
- [ ] Port keybinding behavior from `keybindings.mjs` and `codemirror.mjs`: Ctrl/Alt+Enter eval, Ctrl/Alt+. stop, Alt+w/Alt+q `$` jumps, comment toggle events, and document intentional differences for Vim/Emacs/VSCode/Helix modes if they are not implemented natively.
- [ ] Port autocomplete/tooltip behavior against Rudel's generated reference surface and, later, Strudel's `doc.json`: generic function docs, sound/bank/chord/scale/mode contextual completions, synonym docs, Ctrl-hover tooltips, and `noAutocomplete` filtering.
- [ ] Port theme and draw-theme coupling from `themes.mjs`: editor theme selection should also update the visual draw theme (`background`, `foreground`, `gutterForeground`, etc.) so `pianoroll`, `pitchwheel`, and `spiral` default colors match Strudel.
...
- [ ] Match inline visual widget registration from `codemirror/widget.mjs`: `_pianoroll`, `_punchcard`, `_spiral`, `_scope`, `_pitchwheel`, and `_spectrum` create editor-owned canvas widgets and then call the corresponding pattern method with the injected id and canvas context.
- [ ] Match `draw/pianoroll.mjs`: `pianoroll`, `_pianoroll`, `punchcard`, `_punchcard`, and `wordfall`, including `cycles`, `playhead`, `overscan`, `hideNegative`, `vertical`, `labels`, `flipTime`, `flipValues`, `smear`, `fold`, active/inactive colors, playhead line, note/sample value mapping, velocity/gain alpha, `color`/`label`/`activeLabel`, autorange, and id/tag filtering.
- [ ] Match `draw/pitchwheel.mjs`: pitch-circle rendering from `getFrequency`, root/EDO ring, degree labels, `hapcircles`, `circle`, `thickness`, `hapRadius`, `mode` (`flake` vs `polygon`), margin, event color/velocity/gain alpha, and `_pitchwheel` inline widget behavior.
- [ ] Match `draw/spiral.mjs`: spiral segment geometry, `stretch`, `size`, `thickness`, `cap`, `inset`, playhead segment, `padding`, `steady`, active/inactive colors, `colorizeInactive`, fade behavior, draw-time window handling, and `_spiral` inline widget behavior.
...
- [ ] Match `draw/color.mjs`: CSS named color table and `convertColorToNumber` / `convertHexToNumber` behavior where Rudel exposes color values or uses them in visuals.
...
- [ ] Add visual parity tests: pure geometry/unit tests for pianoroll value/time mapping, pitchwheel angle mapping, spiral coordinates/fade, color conversion, and scheduler draw-window memory; plus scripted UI or screenshot smoke tests for inline widgets and animation repainting.
