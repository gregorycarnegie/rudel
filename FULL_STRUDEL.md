...
- [~] Add audio golden tests where deterministic output is possible, and smoke tests for real-time outputs where exact samples are not stable. The audio-golden methodology is established: `crates/rudel-dsp/tests/zzfx_golden.rs` compares rudel's ZZFX `build_samples` against the real superdough `buildSamples` **sample-for-sample** (14 cases, generated by `tools/oracle/gen_zzfx_oracle.mjs`, the first node→Rust audio oracle alongside the existing reference-formula DSP tests for distortion/filters/drums). Still pending: extending deterministic goldens to the other synth/effect paths and adding real-time smoke tests.
