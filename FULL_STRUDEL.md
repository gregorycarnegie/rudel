...
- [~] Match Strudel's currying, registration, alias, and method-chaining behavior. — Method-chaining and registration are matched: `kpattern_methods!` (`generated.rs`) registers every `Pattern.prototype` transform as a chainable KPattern method, and controls are registered from rudel-core's `control_builders` registry by `extend_control_entries` (the analog of Strudel's `register`). Aliases are matched: camelCase method aliases via the macro's `camel_*` groups, and control-name aliases via `control_name`. Standalone (curried-style) forms are provided for the common transforms by `register_pattern_fns!` (`fast(2, pat)` == `pat.fast(2)`, pattern supplied last) and, for the higher-order combinators, by `register_standalone_callbacks` (`jux(rev, pat)`, `every(4, f, pat)`, `superimpose`/`sometimes`/`often`/`rarely`/`off`/`within`), all with hap-for-hap equivalence tests. Intentionally different: Koto has no partial application, so only fully-applied standalone forms exist (no `fast(2)` returning a function, and callback args must be a function like `rev` or `|x| x.fast(2)`). Still partial: the long tail of standalone forms (`firstOf`/`lastOf`/`chunk`, `inside`/`outside`, `juxBy`/`sometimesBy`, `when`, the `almost*` family, and multi-word/camelCase transforms) remains method-only.
