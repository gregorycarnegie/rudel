...
- [~] Add a headless-browser (or `OfflineAudioContext` polyfill) audio oracle so the Web Audio-rendered superdough paths can be golden-tested sample-for-sample, not just smoke-tested. **Infrastructure established + first path converted.** `tools/oracle/gen_biquad_oracle.mjs` renders a real Web Audio graph through an `OfflineAudioContext` using [`node-web-audio-api`](https://github.com/ircam-ismm/node-web-audio-api) — a faithful native implementation of the Web Audio API in Node, the `OfflineAudioContext`-polyfill route the item calls for (no headless browser needed). It drives a unit impulse through a `BiquadFilterNode` and dumps the impulse response; `crates/rudel-dsp/src/tests/filters.rs::biquad_impulse_response_matches_webaudio` rebuilds it with Rudel's `Biquad` and compares **sample-for-sample** (5 cases, within 2e-4 f32-vs-f64), so the `BiquadFilterNode` path is now a true WebAudio golden rather than a smoke test. `node-web-audio-api` is declared in `tools/oracle/package.json`. Stays `[~]` for the remaining WebAudio-rendered paths: only `bandpass`/`notch` are goldened (their Q is linear in both WebAudio and the RBJ cookbook, matching Rudel exactly), while `lowpass`/`highpass` use WebAudio's dB-Q convention and would need a convention bridge; `OscillatorNode` cannot be sample-matched because Rudel's audible oscillators are intentionally naive (not the browser's band-limited wavetable oscillators), so they remain on behavioral smoke tests by design; and reverb/delay/phaser/vowel goldens through this same OfflineAudioContext harness are the natural next extensions now that the infrastructure exists.
